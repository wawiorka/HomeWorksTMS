...
INSERT INTO employees (id_name, position, department, salary)
VALUES 
...
SELECT id_name FROM employees WHERE position = 'Manager';

--● 7Найдите всех сотрудников, у которых зарплата больше 5000 долларов.
SELECT id_name FROM employees WHERE salary > 5000;

--● 8Найдите всех сотрудников, которые работают в отделе "Sales".
SELECT id_name FROM employees WHERE department = 'Sales';
...
RETURNS TABLE (get_id_name VARCHAR) AS $$
BEGIN
    RETURN QUERY  -- из-за отсутствия этой строки долгое время не получалось вывести результат
    SELECT id_name FROM employees WHERE position ILIKE x_position;
END;
$$ 
LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY
    SELECT id_name FROM employees WHERE salary > $1;
END;
$$ 
LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY  
    SELECT id_name FROM employees WHERE department = $1;
END;
$$ 
LANGUAGE plpgsql;
